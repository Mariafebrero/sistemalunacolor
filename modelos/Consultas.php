...
Class Consultas
{
	...
	{
		$sql="SELECT IFNULL(SUM(total_compra),0) as total_compra FROM tbl_compras WHERE DATE(fecha_compra)=curdate()";
		return ejecutarConsulta($sql);
	}

	public function totalventahoy()
	{
		$sql="SELECT IFNULL(SUM(total),0) as total_venta FROM tbl_facturas WHERE DATE(fecha_factura)=curdate()";
		return ejecutarConsulta($sql);
	}

	public function comprasultimos_10dias()
	{
		$sql="SELECT CONCAT(DAY(fecha_compra),'-',MONTH(fecha_compra)) as fecha,SUM(total_compra) as total FROM tbl_compras GROUP by fecha_compra ORDER BY fecha_compra DESC limit 0,10";
		return ejecutarConsulta($sql);
	}

	public function ventasultimos_12meses()
	{
		$sql="SELECT DATE_FORMAT(fecha_factura,'%M') as fecha,SUM(total) as total FROM tbl_facturas GROUP by MONTH(fecha_factura) ORDER BY fecha_factura DESC limit 0,10";
		return ejecutarConsulta($sql);
	}
}
